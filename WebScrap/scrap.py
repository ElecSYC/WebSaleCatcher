...
r = requests.get(
    'https://listado.mercadolibre.com.ec/{}#D[A:{}]'.format(string.replace(' ', '-'), string))
contenido = r.content
...
for page in range(0, last_page_modified):
    ...
    initial_result = requests.get(
        'https://listado.mercadolibre.com.ec/{}_Desde_{}_NoIndex_True'.format(string.replace(' ', '-'), (page*50)+1))
